...
LRESULT XFreeToolBarReflector::OnToolTip( INT32, LPNMHDR lParam, BOOL &bHandled  )
{
	...
	//UINT32 id = lpTooltip->hdr.idFrom - WM_USER - TOOLBAR_IMAGE_INDEX_BEGIN;
	//UINT32 id = (UINT32)::SendMessage( lpttt->hdr.hwndFrom, TB_COMMANDTOINDEX, lpttt->hdr.idFrom, 0);
	//UINT32 id = lpTooltip->hdr.idFrom;
	//if( id == CHEVRON_BUTTON_ID || id == FILLED_BUTTON_ID )
	//{
	//	return XSUCCESS;
	//}

	//XToolbarItem *pItem;
	//pItem = m_pXConfig->MapBtnConfFromID( id );

	//if( NULL != pItem )
	//	lpTooltip->lpszText = const_cast< TCHAR* >( pItem->GetHintMsg().c_str());
	//else
	//{
	//	lpTooltip->lpszText = const_cast< TCHAR* >( m_pXConfig->m_Settings.m_wordfindHint.c_str());
	//}
...
INT32 XFreeToolBar::SetFonts()
{
	...
	for( INT32 i = 0; i < m_pXConfig->m_Settings.m_fonts.size(); i++ )
	{
...
LRESULT XFreeToolBar::OnChevronPushed(INT32, LPNMHDR pNmHdr, BOOL& bHandled)
{
	...
	//INT32 nBtnCount = m_pXConfig->m_BtnConfs.size();
	POINT ChevronPt;
	...
	//INT32 nAllBtnCount = m_pXConfig->m_BtnConfs.size();
...
//BOOL g_bMouseLeaveHooked = FALSE;
//BOOL g_bMouseIsLeaved = FALSE;
////Catch mouse leave event 
//LRESULT XFreeToolBar::OnHookMouseLeave( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled )
//{
//	g_bMouseLeaveHooked = FALSE;
//	g_bMouseIsLeaved = TRUE;
//	SendMessage( m_hWnd, WM_PAINT, 0, 0 );
//	//ATLTRACE( "Mouse leaved\r\n" );
//	return 0;
//}
...
LRESULT XFreeToolBar::OnHookMouseMove(UINT uMsg, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
{
	...
	//ATLTRACE( "Mouse moved\r\n" );

	//if( FALSE == g_bMouseLeaveHooked )
	//{
	...
	//	g_bMouseLeaveHooked = TRUE;
	//	g_bMouseIsLeaved = FALSE;
	//}
	...
	for(INT32 i = 0; i < m_pXConfig->m_splitters.size(); i++)
	{
		//m_pXConfig->m_splitters[i]->GetSplitterRegion( this, &rcBtn );
...
//{
//	bHandled = FALSE;
//	XLOG ( ( 0, _T( "Enter OnHookRebarSize\r\n" ) ) );
////	if ( FALSE == m_bToolbarInited )
////		return 0;
////
////	INT32 nLeftBtnWidth;
...
////	XLOG ( ( 0, _T( "OnHookRebarSize DepartToolBar\r\n" ) ) );
////	DepartToolBar();
////	CalcXFreeBarWidth();
////
...
//CBrush g_BkBr;
//LRESULT XFreeToolBar::OnHookEraseBkGnd(UINT uMsg, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
//{
//	bHandled = FALSE;
//	//ScreenToClient( &rcToolBar );
//	if( NULL == g_BkBr.m_hBrush )
//	{
//		g_BkBr.CreateSysColorBrush( COLOR_SCROLLBAR );
//	}
//	//lParam = (LPARAM)(HBRUSH)g_BkBr;
//	//OldBr = DrawDC.SelectBrush( BkBr );
//
//	//DrawDC.FillRect( &rcToolBar, BkBr );
//
//	bHandled = FALSE;
//	return 0;
//}
...
LRESULT XFreeToolBar::OnHookPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	GetWindowRect( &rcToolBar );
	ScreenToClient( &rcToolBar );
	//BkBr.CreateSysColorBrush( COLOR_BTNFACE );
	//OldBr = DrawDC.SelectBrush( BkBr );
	//DrawDC.FillRect( &rcToolBar, BkBr );
...
LRESULT XFreeToolBar::OnHookLButtonUp(UINT uMsg, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	//BOOL bHandledTest;
	//OnButton( NULL, MapBtnIDFromIndex( nHit ),m_hWnd,  bHandledTest );
	WhenLButtonUp();
	Invalidate(0);
	return S_OK;
}
...
INT32 XFreeToolBar::DepartToolBar()
{
	...
	::ClientToScreen( m_hWnd, ( LPPOINT )&rcToolBar );
	::ClientToScreen( m_hWnd, ( ( LPPOINT )&rcParent  + 1 ) );
	::ClientToScreen( ::GetParent( m_hWnd ), ( LPPOINT )&rcParent );
	::ClientToScreen( ::GetParent( m_hWnd ), ( ( LPPOINT )&rcParent  + 1 ) );

	//if( rcParent.right - rcParent.left >= ::GetSystemMetrics( SM_CXSCREEN ) )
	//{
	//	rcToolBar.right = rcToolBar.left + ::GetSystemMetrics( SM_CXSCREEN );
	//}

	nClientWidth = rcToolBar.right - rcToolBar.left;
	nClientWidth += rcToolBar.left - rcParent.left;

	ATLTRACE( "Parent left %d, Parent right %d, Tool bar left %d Tool bar right %d\n", 
		rcParent.left, rcParent.right, rcToolBar.left, rcToolBar.right );
...
*/
	...
	if(m_nChevBtnIndex != -1)
	{
		...
		TBInfoSetter( this, FILLED_BUTTON_ID, TRUE ).size( nClientWidth - nLeftBtnWidth - CHEVRON_BUTTON_WIDTH - nRightBtnWidth - TOOLBAR_RESERVE_SPACE);
...
LRESULT XFreeToolBar::OnHookSize(UINT32 uMsg, UINT wParam, LPARAM lParam, BOOL &bHandled)
{
	...
	ATLTRACE( _T( "WM_SIZE wParam is 0x%0x\n" ), wParam );
	if( wParam == SIZE_MINIMIZED || wParam == SIZE_MAXSHOW || wParam == SIZE_MAXHIDE )
	{
		return 0;
	}

	TuneToolBarSizeInBorder();
	//::SetTimer( m_hWnd, TUNE_TOOLBAR_WIDTH_TIMER, 300, NULL );
	return 0;
}

INT32 XFreeToolBar::TuneToolBarSizeInBorder()
{
	INT32 nLeftBtnWidth;
	INT32 nRightBtnWidth;
	INT32 nMinLeftWidth;
	...
	XLOG ( ( 0, _T( "Enter TuneToolBarSizeInBorder\r\n" ) ) );
	if ( FALSE == m_bToolbarInited )
		return 0;

	GetClientRect( &rcToolBar );
	nSizedWidth = rcToolBar.right - rcToolBar.left;

	XLOG ( ( 0, _T( "The border width is %d, the screen width is %d\r\n" ), nSizedWidth, ::GetSystemMetrics( SM_CXSCREEN ) ) );
	nLeftBtnWidth = CalcFixedLeftSideBtnWidth();
	nRightBtnWidth = CalcFixedRightSideBtnWidth();
...
#define OTHER_FILLED_SPACE 10
	...
	//if( nLeftBtnWidth == 0 )
	//{
	//	::MessageBox( NULL, _T( "Left Button width is 0\r\n" ), NULL, 0 );
	//}
...
#define COMBOBOX_HEIGHT 22  
	...
	XLOG( ( 0, _T( "MinLeftWidth is %d Combobox left is %d, right is %d\r\n" ), nMinLeftWidth, rcCombo.left, rcCombo.right ) );

	//if( nSizedWidth < nMinLeftWidth + ( rcCombo.right - rcCombo.left ) )
	//{
	...
	XLOG ( ( 0, _T( "TuneToolBarSizeInBorder new combobox left is %d, right is %d\r\n" ), rcCombo.left, rcCombo.right ) );
	CalcXFreeBarWidth( );
	DepartToolBar();
	return 0;
}
...
LRESULT XFreeToolBar::OnButton( WORD /*wNotifyCode*/, WORD wID, HWND hWndCtl, BOOL& bHandled )
{
	...
	//if( nIndex < m_pXConfig->m_BtnConfs.size() ) //Transact the all command of buttons 
	{
...
BOOL XFreeToolBar::SubclassRebarHost()
{
	...
	//ATLASSERT( NULL != m_ctrlRebar.m_hWnd );
	if( m_ctrlRebar.m_hWnd == NULL )
		return FALSE;

	//XLOG( ( 0, "XFreeToolBar::SubclassRebarHost The windows handle of the rebar is 0x%x\n", m_ctrlRebar.m_hWnd ) );
...
BOOL XFreeToolBar::UnsubclassRebarHost()
{
	...
	//static INT32 nEntried = 0;
	//nEntried ++;
	//ATLASSERT( nEntried <= 1 );
	...
	else
	{
		if( NULL != m_RebarContainer.m_pfnSuperWindowProc )
		{
			...
			{	
				...
				//ATLASSERT( ( LONG_PTR )m_RebarContainer.m_pfnSuperWindowProc == ::GetWindowLongPtr( m_ctrlRebar.m_hWnd, GWLP_WNDPROC) );
			}
		}
	}
...
#define CHEVRON_FUNC_BUTTON_COUNT 2
	...
	if(nNewAdded > 0)
	{
		for( INT32 i = 0; i < nNewAdded; i++ )
		{
			...
			//m_pXBtnInfos[ nBtnCount + i ].pBtnData->iBitmap = (nAllButtonCount - nNormalButtonCount + i) % m_pXConfig->m_Settings.m_highlight.m_colors.size();
	...
	//HideButton( m_pXConfig->MapBtnConfIndexFromCmd(_T("ShowFindButtons")), bHide ); //Find the command of the XToolbarItem that is "ShowFindButtons".
...
//INT32 XFreeToolBar::DestroyImageLists()
//{
//	HIMAGELIST hNormImageList;
//	HIMAGELIST hHotImageList;
//	HIMAGELLST hDisbImageList;
//
//	hNormImageList = ( HIMAGELIST )::SendMessage( m_hWnd, TB_GETIMAGELIST, 0, 0L );
//	hHotImageList = ( HIMAGELIST )::SendMessage( m_hWnd, TB_GETHOTIMAGELIST, 0, 0L );
//	hDisbImageList = ( HIMAGELIST )::SendMessage( m_hWnd, TB_GETDISABLEDIMAGELIST, 0, 0L );
//
//	if( NULL != hNormImageList )
//	{
//		ATLASSERT( TRUE == ImageList_Destroy( hNormImageList ) );
//	}
//	
//	if( NULL != hHotImageList )
//	{
//		ATLASSERT( TRUE == ImageList_Destroy( hHotImageList ); );
//	}
//
//	if( NULL != hDisbImageList )
//	{
//		ATLASSERT( TRUE == ImageList_Destroy( hDisbImageList ); );
//	}
//	return XSUCCESS;
//}
//
//INT32 XFreeToolBar::LoadImageLists( LPCTSTR lpszNormImageFile, LPCTSTR lpszHotImageFile, LPCTSTR lpszDisbImageFile, HINSTANCE hInstance, int cx, int cGrow, COLORREF crMask, UINT uType, UINT uFlags )
//{
//	HIMAGELIST hNormImageList;
//	HIMAGELIST hHotImageList;
//	HIMAGELLST hDisbImageList;
//	DestroyImageLists();
//
//	hNormImageList = ImageList_LoadImage( hInstance, lpszNormImageFile, cx, nGrow, crMask, uType, uFlags);
//	hHotImageList = ImageList_LoadImage( hInstance, lpszHotImageFile, cx, nGrow, crMask, uType, uFlags);
//	hDisbImageList = ImageList_LoadImage( hInstance, lpszDisbImageFile, cx, nGrow, crMask, uType, uFlags);
//
//	if( NULL == hNormImageList || 
//		NULL == hHotImageList ||
//		NULL == hDisbImageList )
//	{
//		DestroyImageLists();
//		return XERROR_LOAD_IMAGELIST;
//	}
//
//	ATLASSERT( TRUE == ::SendMessage( m_hWnd, TB_SETIMAGELIST, ( WPARAM )hNormImageList, 0L ) );
//	ATLASSERT( TRUE == ::SendMessage( m_hWnd, TB_SETHOTIMAGELIST, ( WPARAM )hHotImageList, 0L ) );
//	ATLASSERT( TRUE == ::SendMessage( m_hWnd, TB_SETDISABLEDIMAGELIST, ( WPARAM )hDisbImageList, 0L ) );
//	return XSUCCESS;
//}
...
INT32 XFreeToolBar::GetToolTipInfo( WORD wID, TCHAR **lppszTooltip )
{
	...
	//ATLTRACE("Button State is hide? %s, Chevron is show ? %s \n",
	//	TmpBtnInfo.fsState & TBSTATE_HIDDEN ? "Hide" : "Show",
	//	m_pChevDrop->m_hWnd ? "Show" : "Hide");
	//if( NULL == m_pChevDrop )
	//{
	//	bChevShowed = FALSE;
	//}
	//else if( NULL == m_pChevDrop->m_hWnd )
	//{
	//	bChevShowed = FALSE;
	//}
	//else if( !m_pChevDrop->IsWindowVisible() )
	//{
	//	bChevShowed = FALSE;
	//}
	//else
	...
	//if( !bChevShowed && TmpBtnInfo.fsState & TBSTATE_HIDDEN )
	//{
	//	return XSUCCESS;
	//}
	...
	//else
	//{
	//	*lppszTooltip = const_cast< TCHAR* >( m_pXConfig->m_Settings.m_wordfindHint.c_str() );
	//}
	return XSUCCESS;
}