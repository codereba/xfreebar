...
void CALLBACK CDrawButton_TimerProcRepeat (HWND hWnd, UINT, UINT, DWORD)
{
    ::SendMessage (::GetParent (hWnd), WM_COMMAND, MAKELONG (GetDlgCtrlID (hWnd), s_wNotify), (LPARAM)hWnd);
}

/////////////////////////////////////////////////////////////////////////////
DWORD CDrawButton::Click (CWindow Wnd, CPoint pt, UINT nIDRepeat) const
{
    ...
    if ( DB_GETTYPE(m_wStyle) == DB_UPDOWN )
    {
        ...
        if ( nIDRepeat != 0 )
        {
            ...
            if ( !Wnd.SetTimer (nIDRepeat, 100, &CDrawButton_TimerProcRepeat) )
            {
                nIDRepeat = 0;
            }
        }
...
/////////////////////////////////////////////////////////////////////////////
void CALLBACK CDrawButton_TimerProcMouseOver (HWND hWnd, UINT, UINT, DWORD)
{
    CPoint pt;
...
/////////////////////////////////////////////////////////////////////////////
void CDrawButton::CheckForMouseOver (CWindow Wnd, CPoint pt)
{
    if ( DB_ISFLAT(m_wStyle) )
    {
        if ( !DB_ISOVER(m_wStyle) && PtInRect (pt) )
        {
            if ( Wnd.SetTimer (54321, 300, &CDrawButton_TimerProcMouseOver) )
            {
                m_wStyle |= DB_OVER;
