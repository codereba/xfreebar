#ifndef __XMLOBJ_H_6BC1EE96_0B6F_4ad6_B91E_4D70D2FCBDD0
#define __XMLOBJ_H_6BC1EE96_0B6F_4ad6_B91E_4D70D2FCBDD0
...
extern zstring gAppendCommand;
extern zstring gAppendCommandDisplay;
extern bool g_DO_HTML;// some global variables :-(
extern _bstr_t bandUUID;
extern _bstr_t alertTitle;
extern _bstr_t uninstallMsg;
extern _bstr_t urlAfterUpdate;

extern _variant_t varEmpty;
extern std::map<zstring,zstring> gLastSearchWords;
extern _bstr_t gurl;
 ...
 #include "Toolband7\startAutoLogin.cpp"
    ...
    struct WebJump : public Command
    {
      ...
      void do_apply(CBandToolBarCtrl* ctrl,zstring s)
      {
        ...
        _variant_t postData = _T("");//getPostData((const TCHAR*)replaceUrl(post));
  ...
  struct ToolbarItem : public XmlObject
  {
    ...
    virtual BOOL StopMouseEvents(CBandToolBarCtrl * ctrl = NULL) {return FALSE;} // used to disalbe mouse input for TEXT control
...
#define SPLITTER_WIDTH 11
	  ...
	  Splitter() : pEdit(0), m_oldx(0)
	  {
	  }

	  BOOL getSplitRc(CBandToolBarCtrl *ctrl, LPRECT prc)
	  {
		return ctrl->GetItemRect(m_id, prc);
	  }

	  virtual void onOptionChange(CBandToolBarCtrl *ctrl)
	  {
		  Separator::onOptionChange(ctrl);
		  //for(int i = 0; i< m_toolbar->m_toolbarItems.size(); i++)
		  //{
			 // if(m_toolbar->m_toolbarItems[i].m_id == editname)
			 // {
				//  if(editname == it->getName())
				//  {
				//	  pEdit = (Combo::ICBandEditCtrl*)it;
				//	  ctrl->m_toolbar->m_splitters.push_back(this);
				//	  break;
				//  }
			 //}
		  //}
		  //TBBUTTONINFO tbi;
		  //ctrl->GetButtonInfo(m_id, &tbi);
		  //tbi.fsState |= TBSTATE_ENABLED;
		  //tbi.dwMask |= TBIF_STATE;
		  //ctrl->SetButtonInfo(m_id,&tbi);
		  for(Toolbar::IBandEdits it(ctrl);it;it++)
		  {
			  if(editname == it->getName())
			  {
				  pEdit = (Combo::ICBandEditCtrl*)it;
				  ctrl->m_toolbar->m_splitters.push_back(this);
				  TBInfo(ctrl, m_id).size(SPLITTER_WIDTH).text(_T(""));
				  break;
			  }
		  }
	  }

	  virtual BOOL StopMouseEvents(CBandToolBarCtrl *ctrl) 
	  {
		  if(!pEdit)
			  return FALSE;
		  POINT curPoint;
		  ::GetCursorPos(&curPoint);//::GetMessagePos();
		  int nMove = curPoint.x - m_oldx;

		  //AtlTrace("%s\n", curPoint.x);
		if(nMove > 50 || nMove < -50)
			nMove = 0;
		  ...
		  if(m_oldx)
		  {
			  CRect rcEdit;
			  CRect rcTmp;
			  ctrl->GetItemRect(pEdit->m_combo->m_id, &rcEdit);

			  rcEdit.right += nMove;
			  pEdit->MoveWindow(rcEdit);

			  TBInfo(ctrl,pEdit->m_combo->m_id).size(rcEdit.Width()).text(_T("")); 
			  ctrl->calculateWidth();
		  }
		  m_oldx = curPoint.x;
		  return TRUE;
	  ...
	  {
		  ToolbarItem::determine(p);
		  XML::determineMember<>(editname,_T("editname"),p);
	  }
  };
  virtual void determine(XmlParserProxy& p)
  {
	...
	XML::determineMember<Splitter>(m_toolbarItems, _T("SPLITTER"),p);
...
bool checkFiles( Toolbar* &m_toolbar, bool breadreg = true);

#endif //__XMLOBJ_H_6BC1EE96_0B6F_4ad6_B91E_4D70D2FCBDD0