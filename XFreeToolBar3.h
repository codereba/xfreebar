...
class XFreeToolBar;
class XToolBarHost;
class CXFreeToolBarReflector :
	public CWindowImpl<CXFreeToolBarReflector, CToolBarCtrl>, 
	public CCustomDraw<CXFreeToolBarReflector>
	//public CCommandBarCtrlImpl<CXFreeToolBarReflector, CCommandBarCtrlBase>
{
public:
	CXFreeToolBarReflector(void){}
	virtual ~CXFreeToolBarReflector(void)
	{
		if(::IsWindow(m_hWnd))
			DestroyWindow();
		m_pToolBarHost = NULL;
...
#define REFLECT_WND_CLASS_NAME _T("XFreeBarReflector")
	...
	BEGIN_MSG_MAP_EX(CXFreeToolBarReflector)
		...
		//	return TRUE;
		//}
		CHAIN_MSG_MAP( CCustomDraw<CXFreeToolBarReflector> )
...
#define MODE_FREE_WIDTH 0x01
	...
	INT32 BuildToolBar();
	bool parseXML(XString& path, ToolbarConfig *pXConfig);
	// load XML files, other media files and check that we load them all
	bool ParseToolbarFromXML( ToolbarConfig* &pXConfig, bool breadreg = true);
